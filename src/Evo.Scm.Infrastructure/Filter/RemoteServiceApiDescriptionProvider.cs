    ...
    public void OnProvidersExecuting(ApiDescriptionProviderContext context)
    {
        ...
        foreach (var apiResponseType in GetApiResponseTypes())
        {
            foreach (var result in context.Results.Where(x => x.IsRemoteService()))
            {
                ...
                if (apiResponseType.StatusCode == (int)HttpStatusCode.OK)
                {
                    System.Type voidType = typeof(ResultMessage<>).MakeGenericType(typeof(string));
                    result.SupportedResponseTypes.ReplaceOne(x => x.StatusCode == (int)HttpStatusCode.OK,
                       x => new ApiResponseType()
                       {
                           ...
                           ModelMetadata = _modelMetadataProvider.GetMetadataForType(x.Type.FullName == "System.Void" ? voidType : typeof(ResultMessage<>).MakeGenericType(x.Type)),
                           Type = x.Type.FullName == "System.Void" ? voidType : typeof(ResultMessage<>).MakeGenericType(x.Type),
