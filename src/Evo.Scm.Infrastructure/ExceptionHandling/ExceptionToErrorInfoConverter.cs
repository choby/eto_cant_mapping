...
public class ExceptionToErrorInfoConverter : IExceptionToErrorInfoConverter, ITransientDependency
{
    ...
    protected virtual RemoteServiceErrorInfo CreateErrorInfoWithoutCode(Exception exception, AbpExceptionHandlingOptions options)
    {
        ...
        {
            return CreateValidationError(exception as AbpValidationException);
        }
        ...
        {
            errorInfo.Message = "服务器出错了";// L["InternalServerErrorMessage"];
        }
    ...
    protected virtual RemoteServiceErrorInfo CreateEntityNotFoundError(EntityNotFoundException exception)
    {
        ...
        {
            return new RemoteServiceErrorInfo($"请求的数据未找到,{exception.Id}",exception.Message, ExceptionCodes.请求失败);
        }

        return new RemoteServiceErrorInfo(exception.Message);
    }

    protected virtual RemoteServiceErrorInfo CreateValidationError(AbpValidationException exception)
    {
        ...
        errorInfo.Code = ExceptionCodes.请求数据校验失败;
        errorInfo.ValidationErrors = GetValidationErrorInfos(exception as IHasValidationErrors);

        errorInfo.Message = errorInfo.ValidationErrors?[0].Message ?? "请求的数据不符合业务要求，请检查"; //L["ValidationErrorMessage"];
        return errorInfo;
    }
    ...
    protected virtual RemoteServiceErrorInfo CreateDetailedErrorInfoFromException(Exception exception, bool sendStackTraceToClients)
    {
        ...
        RemoteServiceErrorInfo errorInfo = new RemoteServiceErrorInfo(exception.Message, detailBuilder.ToString());

        if (exception is EntityNotFoundException)
            errorInfo = CreateEntityNotFoundError(exception as EntityNotFoundException);
       
        if (exception is AbpValidationException)
        {
            errorInfo = CreateValidationError(exception as AbpValidationException);
        }
